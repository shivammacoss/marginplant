...
def test_daily_bounds_midweek():
    ...
    assert end == _ist(2026, 5, 18, 23, 59, 59).replace(microsecond=999_000).astimezone(ZoneInfo("UTC"))
...
def test_weekly_bounds_midweek():
    ...
    assert start == _ist(2026, 5, 18, 0, 0, 0).astimezone(ZoneInfo("UTC"))
    assert end == _ist(2026, 5, 24, 23, 59, 59).replace(microsecond=999_000).astimezone(ZoneInfo("UTC"))
...
def test_monthly_bounds():
    ...
    assert end == _ist(2026, 5, 31, 23, 59, 59).replace(microsecond=999_000).astimezone(ZoneInfo("UTC"))
...
def test_monthly_bounds_february_leap():
    ...
    ref = _ist(2024, 2, 10)
    start, end = compute_period_bounds(SettlementCadence.MONTHLY, ref)
    assert end == _ist(2024, 2, 29, 23, 59, 59).replace(microsecond=999_000).astimezone(ZoneInfo("UTC"))
