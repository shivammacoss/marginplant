        ...
        while self._started:
            try:
                ...
                async for msg in self._pubsub.listen():
                    ...
                    if not targets:
                        continue
                    frame = self._frame(channel, payload)
                    if frame is None:
                        continue
                    # Fan out concurrently. ``return_exceptions=True``
                    # keeps one slow client from blocking the others.
                    results = await asyncio.gather(
                        *(safe_send_text(ws, frame) for ws in targets),
...
class UserChannelHub(_BaseHub):
    ...
    # Other publishers under ``user:*`` (e.g. ``user:{id}:risk``,
    # ``user:{id}:marketwatch``) were already being silently dropped
    # pre-refactor because the WS handler never subscribed to them; we
    # preserve that exact behaviour here so this change is a pure
    # plumbing swap with no observable difference for existing clients.
    _ALLOWED_TOPICS = frozenset({"positions", "orders", "wallet", "kyc"})
