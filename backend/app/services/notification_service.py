...
async def _resolve_recipients(
    ...
    own document):
      ...
      1. Every SUPER_ADMIN user — they own the platform, get
         everything.
      2. `source_user.assigned_admin_id` if set — the admin that
         owns this user's pool.
      3. Every broker id in `source_user.broker_ancestry` — root-first
         list including all brokers up to the immediate parent.
      4. `source_user.assigned_broker_id` if set and not already in
         the ancestry list — covers the case where ancestry hasn't
         been backfilled yet on legacy rows.
    ...
    # Super-admins — find ALL of them so multi-super-admin setups
    # don't miss anyone.
    try:
