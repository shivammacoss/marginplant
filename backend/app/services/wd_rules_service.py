...
async def validate_request(
    *,
    ...
    # 5) Mandatory remark — admin can require a free-text note (e.g. UPI
    # ref) before a request is accepted.
    if rule.get("mandatory_remark") and not (user_remark or "").strip():
        raise OrderRejectedError(
            "Please add a remark — admin has marked this required for "
            f"{rule_type.lower()}s",
