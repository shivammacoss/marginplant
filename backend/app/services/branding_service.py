    ...
    *,
...
) -> tuple[PydanticObjectId | None, str]:
    """Return ``(assigned_admin_id, signup_origin)`` for a /register call.

    Resolution order — first hit wins:
      1. If the request hostname matches an admin's ``custom_domain``
         (and BRANDING_ENABLED) → CUSTOM_DOMAIN attribution.
      2. Else if ``referral_code`` is set and resolves to an active
         admin → BRANDED_REFERRAL attribution.
      3. Else → ``(None, PLATFORM)`` (super-admin pool — exactly the
         pre-rollout default).

    Always returns a tuple even when branding is disabled, so the
    ...
    if settings.BRANDING_ENABLED and request_host:
        ...
        if admin is not None:
            return admin.id, ORIGIN_CUSTOM_DOMAIN

    if settings.BRANDING_ENABLED and referral_code:
        try:
            ...
            admin = None
        if admin is not None:
            return admin.id, ORIGIN_BRANDED_REFERRAL

    return None, ORIGIN_PLATFORM
