...
@router.get("")
async def get_status(admin: SuperAdmin) -> dict:
    return {
        "success": True,
        "status": await zerodha_auto_login.get_status(),
    }
...
@router.post("/toggle")
async def toggle(
    body: ToggleBody, request: Request, admin: SuperAdmin
) -> dict:
    try:
...
async def set_schedule(
    body: ScheduleBody, request: Request, admin: SuperAdmin
) -> dict:
    try:
...
@router.post("/test")
async def test_now(request: Request, admin: SuperAdmin) -> dict:
    await _enforce_rate_limit(
